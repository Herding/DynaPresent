var dynaOfInsertion = {
    ...
    stateOfCompare: function(i, j, locs, elems, capacity) {
    	var state = new State(capacity);
    	...
    	state.setColor(i, this.COLOR_OF_COMPARE);
    	state.setColor(j, this.COLOR_OF_COMPARE);
    	state.setMark(elems[i] + '与' + elems[j] + '比较');

    	dynamic.stateList.push(state);
    },

    stateOfInsert: function(i, locs, elems, capacity, cont) {
    	var state = new State(capacity);
    ...
    stateOfExch: function(i, j, elemOfI, elemOfJ) {
    	...
    	state.setMark(elemOfI + '在' + elemOfJ +'左边');
    	dynamic.stateList.push(state);
    },
...
var insertion = {
	...
	present: function() {
        if (!guard.isSafe) {
            ...
            for (var i = 0; i < guard.errors.length; ++i)
                cont += guard.errors[i];
            dynaOfInsertion.drawErrors(cont);
		...
		for (var i = 0; i < len; ++i) {
			...
			clearInterval(dynaOfInsertion.animInterval);
	    	dynaOfInsertion.animInterval = setInterval( function() {

		    	dynaOfInsertion.drawCurrentState(dynamic.stateList[dynaOfInsertion.current]);
		    	if (dynaOfInsertion.current < len - 1)
		    		dynaOfInsertion.current++;
		    	else
		    		clearInterval(dynaOfInsertion.animInterval);
	        }, dynaOfInsertion.transitionTime);
	...
	putVal: function(elem) {
		...
		if (this.size() == this.Max) {
			dynaOfInsertion.stateOfError('元素个数已经达所定义容量');
			return ;
		}
	...
	sort: function() {
		var len = this.size();
		dynaOfInsertion.location(this.size(), this.locs, this.elems, this.MaxOne);
		dynaOfInsertion.stateOfOrigin(this.size(), this.locs, this.elems);
		for (var j, i = 1; i < len; ++i) {
			dynaOfInsertion.stateOfInsert(i, this.locs, this.elems, this.size(), this.elems[i] + '插入到左侧合适位置');
			for (j = i;  j > 0 ; --j) {
				dynaOfInsertion.stateOfCompare(j, j - 1, this.locs, this.elems, this.size());
				if (this.elems[j] >= this.elems[j - 1])
					break ;

				dynaOfInsertion.stateOfExch(j, j - 1, this.elems[j], this.elems[j - 1]);
				this.exch(j, j - 1);
				dynaOfInsertion.location(this.size(), this.locs, this.elems, this.MaxOne);
			}
			dynaOfInsertion.stateOfInsert(j, this.locs, this.elems, this.size(), this.elems[j] + '插入成功');
		}
		dynaOfInsertion.stateOfOrigin(this.size(), this.locs, this.elems);
	}
}