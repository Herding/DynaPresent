...
var dfs = {
	...
	create: function(G, s) {
		...
		dynaOfDFS.location(num, this.locs, this.locsOfEnd);
		dynaOfDFS.getGraph(G);
		dynamic.create();
		dynaOfDFS.stateOfOrigin(this.locs, this.locsOfEnd);
		this.DFS(G, s);
		dynaOfDFS.stateOfOrigin(this.locs, this.locsOfEnd);
	},
	...
	present: function() {
		...
		for (var i = 0; i < len; ++i) {
			...
			clearInterval(dynaOfDFS.animInterval);
	    	dynaOfDFS.animInterval = setInterval( function() {

		    	dynaOfDFS.drawCurrentState(dynamic.stateList[dynaOfDFS.current]);
		    	if (dynaOfDFS.current < len - 1)
		    		dynaOfDFS.current++;
		    	else
		    		clearInterval(dynaOfDFS.animInterval);
	        }, dynaOfDFS.transitionTime);
	...
	DFS: function(G, v) {
		var vertices = G.adjOfV(v),
			num = vertices.length;
		this.marked[v] = true;
		dynaOfDFS.stateOfSelect(v, this.marked, this.edgeTo, true, this.locs, this.locsOfEnd);
		for (var i = 0; i < num; ++i) {
			var w = vertices[i];
			dynaOfDFS.stateOfTravel(v, w, this.marked, this.edgeTo, this.locs, this.locsOfEnd);
			if (!this.marked[w]) {
				this.edgeTo[w] = v;
				dynaOfDFS.stateOfSelect(w, this.marked, this.edgeTo, false, this.locs, this.locsOfEnd);
				this.DFS(G, w);
			}
			else
				dynaOfDFS.stateOfSelect(w, this.marked, this.edgeTo, false, this.locs, this.locsOfEnd);
